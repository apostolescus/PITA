...
class L76X(object):
    ...
    def L76X_Gat_GNRMC(self):
        ...
        for message in split_by_space:
            
            if message.find(speed_infos) >= 0:
                
                try:
                    speed_parser = pynmea2.parse(message)
                    self.speed = speed_parser.spd_over_grnd_kmph
                    self.Status = 1
                except pynmea2.ParseError:
                    print("Parse error speed")
                except AttributeError:
                    print("Atribute Error speed")
                 
            if message.find(lat_lon_infos) >= 0:
               
                try:
                    location_parser = pynmea2.parse(message)
                    self.Lat = location_parser.latitude
                    self.Lon = location_parser.longitude
                    self.Status = 1
                    
                except pynmea2.ParseError:
                    print("Parse error lat/lon")
                except AttributeError:
                    print("Atribute Error lat/lon")
